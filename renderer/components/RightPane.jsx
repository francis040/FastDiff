...
function RightPane({ chunks, text, onTextChange, scrollRef, onScroll, diffRefs, meta }) {
  return (
    <div className="h-full flex flex-col">
      <div className="px-6 py-4 border-b border-gray-200 bg-white">
        <div className="text-xs text-gray-500 mb-2">
          {meta?.path ? `右侧：${meta.path}` : '右侧：粘贴或打开文本'}
        </div>
        <textarea
          value={text}
      ...
      >
        {chunks.map((chunk, index) => (
          <div
            key={`${chunk.type}-${index}-right`}
            ref={(node) => {
              diffRefs.current[index] = diffRefs.current[index] || node;
            }}
          ...
          >
            <div className="flex gap-3 text-sm leading-relaxed">
              <div className="w-12 text-right text-gray-400 select-none pr-1">
                {chunk.rightLineNumber ?? ''}
              </div>
              <div className="flex-1 text-slate-800">
                {chunk.rightSegments?.length ? (
                  <HighlightChunk segments={chunk.rightSegments} />
                ) : (
                  <span className="text-gray-400">{chunk.rightText ? chunk.rightText : ' '}</span>
                )}
              </div>
            </div>
          </div>
